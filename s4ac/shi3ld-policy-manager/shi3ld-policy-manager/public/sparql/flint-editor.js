// based on Flint Sparql Editor written by Stephen Cresswell and Paul Appleby

// adapted to be used with jquery 1.9 (jQuery.browser patch added)
// all the part concerning query submission and result presentation eliminated since not useful


function FlintEditor(container, imagesPath, config) {
	//since from jQuery 1.8 jQuery.browser eliminated this code define it since it was used to deal with IE
	jQuery.browser = {};
	//IE
	...
	function FlintAbout(editor) {
		...
		this.show = function() {
			var aboutText = "<p>"
					...
					+ "<p>Based on Flint SPARQL editor developed by a team at <a href='http://www.tso.co.uk'>TSO</a>.</p>";
			editor.getConfirmDialog().setCloseAction();
	...
	function FlintDialog() {
		...
		this.show = function(title, text, closeOnly) {
			...
			if (!editor.windowClosing) {
				$('#' + flintDiv + '-dialog-title-text').text(title);
				$('#' + flintDiv + '-dialog-text').html(text);
				if (closeOnly) {
					$('#' + flintDiv + '-dialog-okay-button').css('visibility', 'hidden');
				} else {
					$('#' + flintDiv + '-dialog-okay-button').css('visibility', 'visible');
				}
				$('.flint-dialog-body').css('margin-top',
						($('#' + flintDiv + '-editor').position().top + 200) + "px");
				$('#' + flintDiv + '-dialog').css('visibility', 'visible');
			}
		};
		...
		this.display = function(container) {
			var aboutText = "<div id='" + flintDiv + "-dialog' class='flint-dialog'><div class='flint-dialog-body'><div class='flint-dialog-body-container'><h2 id='" + flintDiv + "-dialog-title'><span id='" + flintDiv + "-dialog-close' class='flint-close'></span><span id='" + flintDiv + "-dialog-title-text'>Title goes here</span></h2>"
					+ "<div id='" + flintDiv + "-dialog-text'></div>"
					+ "<div id='" + flintDiv + "-dialog-buttons'><span id='" + flintDiv + "-dialog-close-button' class='flint-close-button''>Close</span><span id='" + flintDiv + "-dialog-okay-button' class='flint-okay-button'>Okay</span></div>"
					+ "</div></div></div>";
			$('#' + container).append(aboutText);

			$('#' + flintDiv + '-dialog-close').click(function() {
				$('#' + flintDiv + '-dialog-okay-button').css('visibility', 'hidden');
				$('#' + flintDiv + '-dialog').css('visibility', 'hidden');
				button = "Close";
				closeAction();
			});
			$('#' + flintDiv + '-dialog-okay-button').click(function() {
				try {
					$('#' + flintDiv + '-dialog-okay-button').css('visibility', 'hidden');
					$('#' + flintDiv + '-dialog').css('visibility', 'hidden');
					button = "Okay";
			...
			$('#' + flintDiv + '-dialog-close-button').click(function() {
				$('#' + flintDiv + '-dialog-okay-button').css('visibility', 'hidden');
				$('#' + flintDiv + '-dialog').css('visibility', 'hidden');
				button = "Close";
				closeAction();
	...
	function FlintStatus() {
		...
		this.display = function(container) {
			$('#' + container).append("<div id='" + flintDiv + "-status'>...</div>");
		};

		this.updateStatus = function() {
			$('#' + flintDiv + '-status').text(
	...
	/*function FlintResults(editor) {
		...
		this.setResults = function(text) {
			...
			if (resultsMode === "Visual"
				...
				results = "<table id='' + flintDiv + '-results-table'><thead><tr>" + devHead
						+ "</tr></thead><tbody>" + devResults + "</tbody></table>";
			} else {
				results = "<textarea id='' + flintDiv + '-results'>" + results + "</textarea>";
			}
			this.showLoading(false);
			try {
				$('#' + flintDiv + '-results-container').html(results);
			} catch (e) {
				editor.getErrorBox().show(e);
			}
		};
		...
		this.showLoading = function(showLoader) {
			if (showLoader) {
				$('#' + flintDiv + '-results-loader').show();
				$('#' + flintDiv + '-results').hide();
			} else {
				$('#' + flintDiv + '-results-loader').hide();
				$('#' + flintDiv + '-results').show();
			}
		};
	...
	/*function FlintMenu(editor) {
		...
		try {
			...
			newMenuItems.push(new FlintMenuItem("EmptyQuery", "Empty policy",
					"New_16x16.png", true, function() {
						editor.clearEditorTextArea();
					}));
			newMenuItems.push(new FlintMenuItem("SelectQuery", "Access condition SPARQL",
					"Properties_16x16.png", true, function() {
						editor.insertSelectQuery();
					}));
			newMenuItems.push(new FlintMenuItem("ConstructQuery", "Access condition RDF",
	...
	function FlintToolbar(editor) {
		...
		try {
			...
			toolbarItems.push(new FlintToolbarItem("New", "New empty policy",
					"New_24x24.png", true, function() {
						editor.clearEditorTextArea();
					}, true));
			toolbarItems.push(new FlintToolbarItem("Select", "Access condition SPARQL",
					"Properties_24x24.png", true, function() {
						editor.insertSelectQuery();
					}, false));
			toolbarItems.push(new FlintToolbarItem("Construct",
					"Access condition RDF", "Key_24x24.png", true, function() {
						editor.insertConstructQuery();
					}, false));
		...
		this.setEnabled = function(id, enabled) {
			...
			for (i = 0; i < toolbarItems.length; i++) {
				if (toolbarItems[i].getId() === id) {
					...
					$("#" + flintDiv + "-toolbar-" + i).attr("class", itemClass);
					break;
				}
			}
		};
		...
		this.display = function(container) {
			...
			var i;
			for (i = 0; i < toolbarItems.length; i++) {
				listItems += "<li id='" + flintDiv + "-toolbar-" + i + "' class='";
			...
			$('#' + container)
					.append("<ul id='" + flintDiv + "-toolbar' class='flint-toolbar'>" + listItems + "</ul>");
			for (i = 0; i < toolbarItems.length; i++) {
				$("#" + flintDiv + "-toolbar-" + i).click(toolbarItems[i].getCallback());
			}
		};
	}
		...
		try {
			...
			this.display = function(container) {
				...
				if (datasetItems.length === 1) {
					$('#' + container)
							.append(
									"<div id='flint-dataset'><h2>RDF serialization format</h2><input disabled='disabled' type=text id='flint-dataset-select' name='kb' value='"
					...
					$('#' + container)
							.append(
									"<div id='flint-dataset' title='Select the endpoint that you wish to query'><h2>RDF serialization format</h2><select id='flint-dataset-select' name='kb'>"
											+ listItems + "</select></div>");
				}
			};
	...
	function FlintCodeEditor(flint, editorMode) {
		//TBD WAY TO PARAMETRIZE (get in the cookie) or to pass from app to flint 
		//this initial query, so that no double definition (in lib and in app.js)
		var initialQuery = "@prefix : <http://example.org/shi3ld/>.";//"PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>\nPREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>\n\nSELECT * WHERE {\n   ?s ?p ?o\n}\nLIMIT 10";
		var cm;
		...
		function cmUpdate() {
			...
			//trigger text modification event
			Backbone.trigger('textChanged');
		...
		this.resize = function() {
			var containerWidth = $('#' + flintDiv + '-editor-tabs-container').width();
			$('#' + flintDiv + '-editor-tabs').css('max-width', containerWidth - 70);
			spaceForTabsCount = Math.floor((containerWidth - 70) / 102);
			tabOffset = 0;
			$('#' + flintDiv + '-editor-tabs li').show();
			//checkTabScroll();
		};
		...
		this.display = function(container) {
			$('#' + container)
					.append(
							"<div id='" + flintDiv + "-editor-container' class='flint-editor-container'>" /*<div id='flint-editor-tabs-container'><div id='flint-scroll-tabs-left'><img src='"
									+ flint.getImagesPath()
									+ "/Previous.png'/></div>"
									...
									+ flint.getImagesPath()
									+ "/Next.png'/></div></div>*/
									+ "<textarea id='" + flintDiv + "-code' name='query' cols='100' rows='1'>"
									+ initialQuery + "</textarea></div>");

			cm = CodeMirror.fromTextArea(document.getElementById(flintDiv + "-code"), {
				mode : editorMode,
				lineNumbers : true,
				indentUnit : 3,
				tabMode : "indent",
	...
	function Flint(container, imagesPath, config) {
		...
		var flint = this, editorId = flintDiv + "-editor", showResultsInSitu = true, cm, createToolbar, createMenu;
		...
		try {
			...
			this.getTitle = function() {
				return "SHI3LD Policy Manager Editor";
			};
			...
			if ($.browser.msie) {
				...
				$('#' + container).append("<div id='" + editorId + "' class='flint-editor'></div>");
			}
...
//			}
			...
			//parametrized function name
			
			var f = function(e, nameDiv) {
				// Resize editing area. Should this be in the
				// FlintCodeEditor object?
				// Will be triggered by window resize and sidebar
				// display/hiding
				if (nameDiv != flintDiv) return;
				var editorWidth = $('#' + flintDiv + '-editor').width();
				//if (!createSidebar.visible()) {
					$('.CodeMirror').css("width", (editorWidth - 10) + "px");
				//} else {*/
				//	$('#' + flintDiv + '-sidebar').css("width", editorWidth / 2 + "px");
				//	$('.CodeMirror').css("width", ((editorWidth / 2) - 25) + "px");
				//}
				cm.resize();
			}
			f.name = flintDiv + 'resizeHandler';
...
//				resultsArea.display(editorId);
			...
			$(window).trigger('resize', flintDiv);
		} catch (e) {
			window.alert(e);
		}
	}
