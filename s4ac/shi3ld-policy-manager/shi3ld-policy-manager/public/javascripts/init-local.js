$(document).ready(function() {
	...
	//-----------------------end Jeditable event handler--------------------------------
	
	//to make the text editor properly visible with twitter bootstrap when corresponding tabs is selected
	$("#editTab").on('shown', function (e) {
		$(window).resize();
		$(".flint-editor-tab-selected").trigger("click");
	});
	
	$("#savePolicy").on('click', function (e) {
		uri = "policies/?policy=" + encodeURIComponent("http://example.org/shi3ld/ap1-DEL");
		$.ajax({
            type: 'POST',
            contentType: "text/turtle",
            url: uri,
            data: flintEd.getEditor().getCodeEditor().getValue(),
